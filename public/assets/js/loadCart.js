...
let currentDeliveryCost = 25; // Default delivery cost
let discount = 0;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (user) {
      ...
      get(cartRef).then((snapshot) => {
        if (snapshot.exists()) {
          ...
          // Check for persisted promocode
          const storedPromocode = localStorage.getItem('promocode');
          if (storedPromocode) {
            applyPromocode(storedPromocode);
            document.getElementById('promocode').value = storedPromocode;
          }
  ...
  // Add event listener for applying promocode
  ...
  document.getElementById('delivery-options').addEventListener('change', function () {
    currentDeliveryCost = parseFloat(this.value);
    updateTotalPrice();
  });
});
...
function updateTotalPrice() {
  ...
  const totalPriceWithDelivery = discountedTotalPrice + currentDeliveryCost;

  document.getElementById('summary-total-price').textContent = originalTotalPrice.toFixed(2);
  document.getElementById('total-price').textContent = totalPriceWithDelivery.toFixed(2);
}

// Function to apply promocode
function applyPromocode(promocode) {
  if (promocode === 'eubebiu10') {
    discount = originalTotalPrice * 0.10;
    localStorage.setItem('promocode', promocode); // Persist promocode
    updateTotalPrice();
  } else {
    alert('Invalid promocode.');
    discount = 0;
    localStorage.removeItem('promocode'); // Remove invalid promocode
    updateTotalPrice();
  }
}
